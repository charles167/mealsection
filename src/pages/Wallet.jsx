...
function Wallet() {
  ...
  const { user, userFetch, setUser } = useAuthContext();
  ...
  const handleSuccess = async (reference) => {
    ...
    toast.loading("Confirming wallet top-up...", { id: "wallet-topup" });

    try {
      const verifyRes = await axios.post(
        `${import.meta.env.VITE_REACT_APP_API}/api/users/verify-paystack`,
        { reference: ref, userId, amount: topupAmount },
        {
          headers: {
      ...
      if (verifyRes.data?.status === "success" || verifyRes.data?.verified) {
        ...
        toast.success(
          verifyRes.data?.credited
            ? `Wallet credited with ₦${Number(
                verifyRes.data?.amount || topupAmount
              ).toLocaleString()}`
